...
const PROHIBITED_TERMS = [
  // English profanity, personal abuse, and common hate slurs.
  ...
  // Hindi/Hinglish and widely used Indian transliterations.
  ...
  // Bengali, Tamil, Telugu, Kannada, Malayalam, Gujarati, and Punjabi transliterations.
  ...
  // Common abusive terms from other widely used languages.
  ...
  // Native-script forms. Unicode escapes keep source encoding stable on Windows.
...
const PROHIBITED_PHRASES = ["click here to win", "guaranteed income", "buy now"];
const CONTENT_BLOCKLIST = ["buy now!!!", "click here to win", "viagra", "guaranteed income"];
...
function checkCommentContent(content) {
  ...
  if (hasProhibitedTerm) {
    return { isAllowed: false, message: "Your comment contains abusive or prohibited language. Please edit it before posting." };
  }
  if (PROHIBITED_PHRASES.some((phrase) => containsWholeTerm(normalized, normalizeForModeration(phrase)))) {
    return { isAllowed: false, message: "Your comment looks like prohibited promotional or spam content. Please edit it before posting." };
  }
  if (THREAT_PATTERNS.some((pattern) => pattern.test(normalized))) {
    return { isAllowed: false, message: "Threatening language is not allowed. Please edit your comment before posting." };
  }
  return { isAllowed: true, message: "" };
}

function findBlockedPhrase(text) {
  const haystack = String(text || "").toLocaleLowerCase("en-IN");
  return CONTENT_BLOCKLIST.find((term) => haystack.includes(term)) || null;
}

module.exports = { checkCommentContent, findBlockedPhrase };
